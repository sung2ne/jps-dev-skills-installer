...
Claude Code 스킬 인스톨러. gstack의 의사결정 스킬과 Superpowers의 실행 스킬만
골라 미니멀하게 설치합니다.

명지전문대학 정보통신공학과 학생들과 본인 연구실용 표준 셋업으로 만들어졌으며,
두 프레임워크를 통째로 깔았을 때 발생하는 의사결정 레이어 중복과 토큰 폭증을 피하면서
양쪽의 핵심 강점만 가져오는 것이 목표입니다.

## 무엇이 설치되나

총 7개 스킬이 `~/.claude/skills/` 아래에 설치됩니다.
...
다음은 일부러 설치하지 않습니다. 필요해지면 개별 폴더만 추가하시면 됩니다.

- gstack의 페르소나 풀세트(`/qa`, `/ship`, `/review`, `/design-*`, `/codex` 등) — 토큰 폭증의 주요 원인
- gstack의 브라우저 데몬, telemetry, brain-sync — bun/Node.js 빌드 의존성 회피
- Superpowers의 `brainstorming`, `requesting-code-review` — gstack 의사결정 레이어와 기능 중복
...
```bash
# 스크립트 다운로드 후
chmod +x install-minimal-claude-skills.sh
./install-minimal-claude-skills.sh
```

또는 dry-run으로 무엇이 일어날지 먼저 확인:

```bash
./install-minimal-claude-skills.sh --dry-run
```
...
| 플래그 | 동작 |
|---|---|
| (없음) | 기본 위치(`~/.claude/skills`)에 설치. 기존 항목과 충돌하면 중단 |
| `--update` | 기존 설치 덮어쓰기. 두 레포의 최신 main 브랜치를 다시 가져옴 |
| `--dry-run` | 실제 변경 없이 어떤 동작이 일어날지 출력만 |
| `--remove` | 설치된 7개 스킬과 gstack 헬퍼 자산 제거 |
...
```bash
./install-minimal-claude-skills.sh --remove
```
...
이 인스톨러 스크립트(`install-minimal-claude-skills.sh`)와 본 README는
