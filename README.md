...
> 실제 사용 흐름·작업 종류별 워크플로우·시나리오 예시는 [사용법.md](사용법.md) 참조.
...
`--with-debug`, `--with-review`, `--with-all` 옵션으로 설치한 경우, 위 블록 대신 스크립트가 설치 마지막에 출력하는 "Available skills" 블록을 그대로 복사해서 쓰세요. 옵션에 따라 포함될 스킬 목록이 자동으로 갱신됩니다.
