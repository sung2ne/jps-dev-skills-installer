...
#
# 사용법:
#   bash install-minimal-claude-skills.sh             # 설치
#   bash install-minimal-claude-skills.sh --dry-run   # 변경 없이 시뮬레이션
#   bash install-minimal-claude-skills.sh --update    # 기존 설치 업데이트
#   bash install-minimal-claude-skills.sh --remove    # 설치본 제거
#   bash install-minimal-claude-skills.sh --target DIR  # 설치 위치 변경
#
...
if [ "$REMOVE" -eq 1 ]; then
  log "제거 모드: $TARGET_DIR 안에서 미니멀 스킬을 제거합니다"
  ...
  log "완료. CLAUDE.md에 추가하셨던 스킬 목록도 직접 정리해 주세요."
  exit 0
fi
...
다음 단계:
  1) Claude Code를 재시작하면 7개 스킬이 로드됩니다.
  ...
  2) 프로젝트의 CLAUDE.md(또는 ~/.claude/CLAUDE.md)에 다음을 추가하면
     ...
     ## Available skills
     - /office-hours, /plan-ceo-review, /plan-eng-review (gstack 의사결정)
     - writing-plans, executing-plans, test-driven-development,
       verification-before-completion (Superpowers 실행)
